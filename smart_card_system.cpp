...
void deluser()				//delete user details
{
	...
	cin>>uid;
	ifstream fin("user_details.txt",  ios::in | ios::app); 
...
class ticket : private user, public bus			//class to get ticket details
{
	...
	public:
		...
		void totalfare(struct route **start)		//generates total fare
		{
...
class bank										//class for payment transaction
{
    char name[20];
    int acno;
    char actype[4];
    ...
    do
	{
        ...
        switch(ch)
		{
            ...
            case 2: b[bi].addbus();                 //to get bus details
                    ...
                    bi++;
                    break;
            case 3: t[ti].start_location();			//to get ticket details and totalfare
					t[ti].end_location();
					b[bi--].userenterbus();
					ti++;
					break;
			...
			case 6: ui=0;							//to delete user details
					...
					ui++;
					break;
			case 7: ai=0;							//for payment transaction
					a[ai].init();
					a[ai].withdraw();
					a[ai].disp_det();
					ai++;
